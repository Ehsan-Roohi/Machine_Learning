...
All six conditions are implemented algebraically with full accommodation `chi=1`. The solver uses a damped, checkpoint-invariant fixed-point iteration of the two exact maps used by ASTR's split boundary update: Eqs. (7a–c) set normal velocity, tangential slip and temperature jump; Eqs. (7b–f) set stress and heat-flux boundary values. The damping changes only the nonlinear iteration path, not the Eq. (7) fixed point. No uncheckpointed wall-memory arrays are used.

## Paper conditions reproduced

- square cavity, homogeneous `z` direction;
- `T0 = 273 K` on all walls;
- top-lid final speed `50 m/s`, reached through a 1000-iteration numerical homotopy; the final steady boundary condition is exactly the paper value;
- full Maxwell accommodation `chi=1`;
- monatomic argon mapping (`R=208.13 J kg^-1 K^-1`), matching the DSMC reference family cited by Rana;
...
- mesh ladder at `Kn = 0.05, 0.10, 0.50` for the paper mesh sizes `N = 40, 50, 75, 100, 200, 400` cells/intervals per direction (ASTR stores `N+1` physical nodes).
...
4. fresh paper-mesh `N=40` smoke run from equilibrium;
