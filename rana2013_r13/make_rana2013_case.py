...
def main():
    ...
    meta={'paper':'Rana et al. JCP 236 (2013) 169-186','paper_kn':a.paper_kn,'mean_free_path_kn_convention':meanfree_kn,'paper_mesh_N_cells_per_direction':a.grid_points,'ASTR_grid_nodes_per_direction':cells+1,'ASTR_intervals_im_jm':cells,'mpi_decomposition':[px,py,1],'mpi_ranks':px*py,'T0_K':T0,'lid_velocity_m_per_s':ULID,'specific_gas_constant_J_kgK':R_ARGON,'gas_mapping':'monatomic argon, matching the DSMC reference family cited by Rana','speed_ratio_U_over_sqrt_theta0':speed_ratio,'Mach':mach,'Reynolds':reynolds,'paper_kn_identity':'Kn=sqrt(gamma)*Ma/Re=mu/(rho*sqrt(theta)*L)','dt':a.dt,'maxstep':a.maxstep,'initial_state':'global equilibrium at rest','moment':'full nonlinear transformed Maxwell R13 Eq. (13)','walls':'Rana Eq. (7a-f), chi=1, isothermal; 1000-iteration homotopy then exact Uwall=50 m/s final condition'}
