...
def main():
    ...
    n1=0
    # Constant lid speed from the first nonlinear iteration, as in the paper.
    ...
    bc1,n3=re.subn(r'uwall\s*=\s*dble\(nstep\)\*deltat(?:\*10\.0d0)?\s*\n\s*if \(uwall > 1\.0d0\) uwall = 1\.0d0',
                   'uwall = 1.0d0',bc0)
    if n2<1 or n3<1: raise RuntimeError(f'Constant-lid patches missing: method={n2}, bc={n3}')
    ...
    checks={
      ...
      'constant_lid_method':n2>=1,
      'constant_lid_bc':n3>=1,
    }
    report={'reference':'Rana, Torrilhon & Struchtrup, JCP 236 (2013), Eqs. (3),(4),(7),(13)',
            'model':'full nonlinear transformed Maxwell R13 Eq. (13), exact wall-map iteration',
