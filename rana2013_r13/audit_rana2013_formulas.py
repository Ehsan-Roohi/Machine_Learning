...
def main():
    ...
    args=ap.parse_args()
    text=(args.astr/'src/methodmoment.F90').read_text()
    bc=(args.astr/'src/bc.F90').read_text()
    static={
      ...
      'constant_lid':('uwall = dble(nstep)*deltat' not in text) and ('uwall = dble(nstep)*deltat' not in bc),
    ...
    for _ in range(200):
        ...
        A=rng.normal(size=(3,3)); sigma=stf2(A)
        q=rng.normal(size=3); gs=rng.normal(size=(3,3,3)); gq=rng.normal(size=(3,3)); gp=rng.normal(size=3)
        ...
        sig=p*s0; qq=p*q0
        gsig=np.einsum('ij,k->ijk',s0,gp2); gqq=np.einsum('i,k->ik',q0,gp2)
        mg,Rg,Dg=closures(rho,p,mu,sig,qq,gsig,gqq,gp2)
        mn=(4/(3*p))*stf3(np.einsum('i,jk->ijk',qq,sig))
        Rn=20/(7*rho)*stf2(sig.T@sig)+64/(25*p)*stf2(np.outer(qq,qq))
        Dn=5*np.sum(sig*sig)/rho+56/5*np.dot(qq,qq)/p
        ...
        C=np.sqrt(2/(np.pi*th))
        stn=-C*(PP*V+qt/5+mtnn/2)
        qn=-C*(2*PP*TJ-PP*V*V/2+th*snn/2+De/15+5*Rnn/28)
        Rtn=C*(6*PP*TJ*V+PP*th*V-PP*V**3-11*th*qt/5-th*mtnn/2)
        mnnn=C*(2*PP*TJ/5-3*PP*V*V/5-7*th*snn/5+De/75-Rnn/14)
    ...
    tol=1e-11
    numerical={k:(v<tol) for k,v in err.items()}
    report={'reference':PAPER,'scope':{'bulk_balances':'Eqs. (1),(3),(4) coefficient fingerprints and Maxwell relaxation coefficients','closures':'full transformed nonlinear Eq. (13), randomized tensor oracle','walls':'Eqs. (7a-f), full accommodation, constructed-residual oracle','geometry':'constant-speed isothermal square lid cavity'},'static_checks':static,'oracle_errors':err,'oracle_tolerance':tol,'oracle_checks':numerical,'all_passed':all(static.values()) and all(numerical.values())}
